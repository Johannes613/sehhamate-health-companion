...
export default function MacroDistributionCard({ protein = {}, carbs = {}, fat = {}, onPress }) {
  ...
  const carbsCurrent = parseFloat(carbs.current || 120);
  const carbsGoal = parseFloat(carbs.goal || 250);
  const proteinCurrent = parseFloat(protein.current || 85);
  const proteinGoal = parseFloat(protein.goal || 150);
  const fatCurrent = parseFloat(fat.current || 45);
  const fatGoal = parseFloat(fat.goal || 67);

  const macros = {
    carbs: { 
      current: carbsCurrent.toFixed(1), 
      goal: carbsGoal.toFixed(1), 
      percentage: Math.round((carbsCurrent / carbsGoal) * 100), 
      color: Colors.activityPurple 
    },
    protein: { 
      current: proteinCurrent.toFixed(1), 
      goal: proteinGoal.toFixed(1), 
      percentage: Math.round((proteinCurrent / proteinGoal) * 100), 
      color: Colors.warning 
    },
    fat: { 
      current: fatCurrent.toFixed(1), 
      goal: fatGoal.toFixed(1), 
      percentage: Math.round((fatCurrent / fatGoal) * 100), 
      color: Colors.danger 
    },
  };
