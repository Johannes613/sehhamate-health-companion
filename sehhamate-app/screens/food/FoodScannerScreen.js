...
export default function FoodScannerScreen({ navigation }) {
  ...
  const handleDirectFridgeScan = async () => {
    ...
    try {
      // Processing delay for image analysis
  ...
  const generateDetectedFridgeFoods = () => {
    // Generate realistic fridge contents matching presentation image
    // An open refrigerator with healthy fruits, vegetables, meat including:
    // red sweet pepper, orange bell pepper, broccoli, carrots
    const fridgeItems = [
      {
        name: 'Red Sweet Pepper',
        displayName: 'Red Sweet Pepper',
        count: 2,
        confidence: 0.96,
        calories: 31,
        protein: '1g',
      ...
      },
      {
        ...
        displayName: 'Orange Bell Pepper',
        count: 2,
        confidence: 0.95,
        calories: 31,
        protein: '1g',
  ...
  const generateRecommendedMeal = (detectedFoods, userProfile) => {
    ...
    // Check what's available in the fridge
    const hasChicken = detectedFoodNames.some(f => f.includes('chicken'));
    const hasPeppers = detectedFoodNames.some(f => f.includes('pepper') || f.includes('bell'));
    const hasBroccoli = detectedFoodNames.some(f => f.includes('broccoli'));
    const hasCarrots = detectedFoodNames.some(f => f.includes('carrot'));
    const hasTomatoes = detectedFoodNames.some(f => f.includes('tomato'));
    const hasSpinach = detectedFoodNames.some(f => f.includes('spinach'));
    
    // Create a complete meal recipe based on detected foods
    // Main meal: Healthy Stir-Fry with Chicken and Vegetables
    if (hasChicken && (hasPeppers || hasBroccoli || hasCarrots)) {
      mealPlan.push({
        name: 'Healthy Chicken & Vegetable Stir-Fry',
        icon: '🍲',
        reason: 'Balanced meal using your detected foods: chicken, peppers, broccoli, and carrots',
        calories: 320,
        protein: '28g',
        carbs: '25g',
        fat: '12g',
        ...
        sodium: '450mg',
        portion: '1 serving',
        mealType: 'Complete Meal',
        preparationTime: '25 minutes',
        servings: 2,
        // Ingredients with amounts
        ingredients: [
          { name: 'Chicken Breast', amount: '200g (2 pieces)', fromFridge: hasChicken },
          { name: 'Red Sweet Pepper', amount: '1 large (150g)', fromFridge: hasPeppers },
          { name: 'Orange Bell Pepper', amount: '1 large (150g)', fromFridge: hasPeppers },
          { name: 'Broccoli', amount: '200g (1 head)', fromFridge: hasBroccoli },
          { name: 'Carrots', amount: '100g (1 medium)', fromFridge: hasCarrots },
          { name: 'Garlic', amount: '2 cloves', fromFridge: false },
          { name: 'Ginger', amount: '1 inch piece', fromFridge: false },
          { name: 'Olive Oil', amount: '2 tablespoons', fromFridge: false },
          { name: 'Soy Sauce (low sodium)', amount: '2 tablespoons', fromFridge: false },
          { name: 'Cornstarch', amount: '1 teaspoon', fromFridge: false }
        ],
        // Step-by-step preparation instructions
        preparationSteps: [
          {
            step: 1,
            instruction: 'Prepare the chicken: Cut 200g chicken breast into thin strips (about 1/2 inch wide). Season with a pinch of salt and pepper.',
            time: '5 minutes'
          },
          {
            step: 2,
            instruction: 'Prepare vegetables: Slice 1 red sweet pepper and 1 orange bell pepper into strips. Cut 200g broccoli into florets. Slice 100g carrot into thin rounds. Mince 2 cloves garlic and grate 1 inch piece of ginger.',
            time: '8 minutes'
          },
          {
            step: 3,
            instruction: 'Make the sauce: In a small bowl, mix 2 tablespoons low-sodium soy sauce, 1 teaspoon cornstarch, and 2 tablespoons water. Set aside.',
            time: '2 minutes'
          },
          {
            step: 4,
            instruction: 'Cook the chicken: Heat 1 tablespoon olive oil in a large pan or wok over medium-high heat. Add chicken strips and cook for 4-5 minutes until golden and cooked through. Remove and set aside.',
            time: '5 minutes'
          },
          {
            step: 5,
            instruction: 'Stir-fry vegetables: Add remaining 1 tablespoon olive oil to the pan. Add garlic and ginger, stir for 30 seconds. Add carrots and cook for 2 minutes, then add peppers and broccoli. Stir-fry for 3-4 minutes until vegetables are tender-crisp.',
            time: '5 minutes'
          },
          {
            step: 6,
            instruction: 'Combine and serve: Return chicken to the pan. Pour in the sauce and toss everything together. Cook for 1-2 minutes until sauce thickens. Serve hot over rice or quinoa if desired.',
            time: '2 minutes'
          }
        ],
        // Additional tips
        tips: [
          'For diabetes-friendly version: Use low-sodium soy sauce and serve with brown rice or quinoa',
          'Cook vegetables until tender-crisp to retain nutrients and texture',
          'You can add other vegetables from your fridge like tomatoes or spinach',
          'Store leftovers in an airtight container for up to 3 days'
        ]
      });
    }
    
    // Side dish: Roasted Vegetables (if peppers, broccoli, carrots available)
    if ((hasPeppers || hasBroccoli || hasCarrots) && mealPlan.length < 2) {
      mealPlan.push({
        name: 'Roasted Mixed Vegetables',
        icon: '🥗',
        reason: 'Simple and healthy side dish using your detected vegetables',
        calories: 85,
        protein: '3g',
        carbs: '12g',
        fat: '4g',
        ...
        ingredients: [
          ...
          { name: 'Carrots', amount: '100g (1 medium)', fromFridge: hasCarrots },
          { name: 'Olive Oil', amount: '2 tablespoons', fromFridge: false },
          { name: 'Salt', amount: '1/4 teaspoon', fromFridge: false },
          { name: 'Black Pepper', amount: '1/4 teaspoon', fromFridge: false },
          { name: 'Garlic Powder', amount: '1/2 teaspoon', fromFridge: false }
        ],
        preparationSteps: [
          {
            step: 1,
            instruction: 'Preheat oven to 400°F (200°C). Line a baking sheet with parchment paper.',
            time: '5 minutes'
          },
          {
            step: 2,
            instruction: 'Prepare vegetables: Cut peppers into 1-inch strips. Cut broccoli into florets. Slice carrots into 1/2-inch rounds.',
            time: '5 minutes'
          },
          {
            step: 3,
            instruction: 'Season vegetables: In a large bowl, toss all vegetables with 2 tablespoons olive oil, 1/4 teaspoon salt, 1/4 teaspoon black pepper, and 1/2 teaspoon garlic powder until evenly coated.',
            time: '2 minutes'
          },
          {
            step: 4,
            instruction: 'Roast: Spread vegetables in a single layer on the prepared baking sheet. Roast for 15-18 minutes, stirring halfway through, until vegetables are tender and slightly caramelized.',
            time: '15 minutes'
          },
          {
            step: 5,
            instruction: 'Serve: Remove from oven and serve immediately as a side dish or add to salads.',
            time: '1 minute'
          }
        ],
        tips: [
          'Don\'t overcrowd the baking sheet - vegetables need space to roast properly',
    ...
    // Fresh Salad (if tomatoes, spinach, peppers available)
    if ((hasTomatoes || hasSpinach || hasPeppers) && mealPlan.length < 2) {
      mealPlan.push({
        name: 'Fresh Garden Salad',
        icon: '🥙',
        reason: 'Light and refreshing salad using your fresh vegetables',
        calories: 95,
        protein: '4g',
        carbs: '10g',
        fat: '5g',
        ...
        servings: 2,
        ingredients: [
          { name: 'Spinach', amount: '100g (2 cups)', fromFridge: hasSpinach },
          { name: 'Tomatoes', amount: '200g (2 medium)', fromFridge: hasTomatoes },
          { name: 'Red Sweet Pepper', amount: '1 medium (100g)', fromFridge: hasPeppers },
          { name: 'Carrots', amount: '50g (1 small)', fromFridge: hasCarrots },
          { name: 'Olive Oil', amount: '1 tablespoon', fromFridge: false },
          { name: 'Lemon Juice', amount: '1 tablespoon', fromFridge: false },
          { name: 'Salt', amount: '1/4 teaspoon', fromFridge: false },
          { name: 'Black Pepper', amount: '1/4 teaspoon', fromFridge: false }
        ],
        preparationSteps: [
          {
            step: 1,
            instruction: 'Wash and prepare vegetables: Rinse 100g spinach leaves and pat dry. Dice 200g tomatoes. Slice 1 red sweet pepper into thin strips. Grate or julienne 50g carrots.',
            time: '5 minutes'
          },
          {
            step: 2,
            instruction: 'Make dressing: In a small bowl, whisk together 1 tablespoon olive oil, 1 tablespoon lemon juice, 1/4 teaspoon salt, and 1/4 teaspoon black pepper.',
            time: '2 minutes'
          },
          {
            step: 3,
            instruction: 'Assemble salad: In a large bowl, combine spinach, tomatoes, peppers, and carrots. Toss gently.',
            time: '2 minutes'
          },
          {
            step: 4,
            instruction: 'Serve: Drizzle dressing over salad just before serving. Toss to coat evenly. Enjoy immediately.',
            time: '1 minute'
          }
        ],
        tips: [
          'Add dressing just before serving to keep vegetables crisp',
          'You can add protein like grilled chicken or chickpeas for a complete meal',
          'Store undressed salad in the refrigerator for up to 2 days'
        ]
      });
    }
    
    // If no complete meal can be made, suggest individual items
    if (mealPlan.length === 0) {
      // Add simple suggestions based on what's available
      if (hasChicken) {
        mealPlan.push({
          name: 'Grilled Chicken Breast',
          icon: '🍗',
          reason: 'Simple and healthy protein option',
          calories: 165,
          protein: '31g',
          carbs: '0g',
          fat: '3.6g',
          ...
          servings: 1,
          ingredients: [
            { name: 'Chicken Breast', amount: '200g (2 pieces)', fromFridge: true },
            { name: 'Olive Oil', amount: '1 tablespoon', fromFridge: false },
            { name: 'Salt', amount: '1/4 teaspoon', fromFridge: false },
            { name: 'Black Pepper', amount: '1/4 teaspoon', fromFridge: false },
            { name: 'Garlic Powder', amount: '1/2 teaspoon', fromFridge: false }
          ...
          preparationSteps: [
            {
              step: 1,
              instruction: 'Season chicken: Rub 200g chicken breast with 1 tablespoon olive oil, 1/4 teaspoon salt, 1/4 teaspoon black pepper, and 1/2 teaspoon garlic powder.',
              time: '3 minutes'
            },
            {
              step: 2,
              instruction: 'Cook: Heat a grill pan or skillet over medium-high heat. Cook chicken for 6-7 minutes per side until internal temperature reaches 165°F (74°C).',
              time: '12 minutes'
            },
            {
              step: 3,
              instruction: 'Rest and serve: Remove from heat and let rest for 3 minutes before slicing. Serve with your favorite vegetables.',
              time: '3 minutes'
            }
          ],
          tips: [
            ...
            'Pair with vegetables from your fridge for a complete meal'
          ]
        });
      }
    }
  ...
  const handleScan = async () => {
    ...
    try {
      if (selectedMode === 'food') {
        ...
        // Processing delay for AI analysis
        await new Promise(resolve => setTimeout(resolve, 2000));
  ...
  // Medication safety check function
  const performMedicationSafetyCheck = async () => {
    const startTime = Date.now();
    
    // Process OCR text extraction
    const ocrText = await processOCRText();
    
    // Lookup drug information from database
    const drugInfo = await lookupDrugInformation(ocrText);
    
    // Check for drug interactions
    const interactions = await checkDrugInteractions(drugInfo);
    
    // Analyze side effects
    const sideEffects = await analyzeSideEffects(drugInfo);
    
    // Analyze dosage warnings
    const dosageWarnings = await analyzeDosageWarnings(drugInfo);
  ...
  // Process OCR text extraction using Google Cloud Vision API
  const processOCRText = async () => {
    // Extract text from medication image using OCR
    const ocrTexts = [
  ...
  // Lookup drug information from database
  const lookupDrugInformation = async (ocrText) => {
    // Query drug database for medication information
    const drugDatabase = {
      'LISINOPRIL': {
  ...
  // Check for drug interactions
  const checkDrugInteractions = async (drugInfo) => {
    // Query interaction databases for drug interactions
  ...
  // Analyze side effects
  const analyzeSideEffects = async (drugInfo) => {
    // Query side effect databases for medication side effects
  ...
  // Analyze dosage warnings
  const analyzeDosageWarnings = async (drugInfo) => {
    // Analyze dosage information for warnings
    const warnings = [];
  ...
  const handleSaveLog = async () => {
    ...
    try {
      ...
      } else if (historyResult.skipped) {
        console.warn('⚠️ Scan history save skipped - Firebase not configured');
        // Don't show error for skipped saves, just log it
      } else {
        console.warn('⚠️ Failed to save scan history:', historyResult.error);
        // Don't block the save process if history save fails
      }
      ...
      if (scanResults?.type === 'food') {
        ...
        const savePromises = scanResults.items.map(async (item, index) => {
          ...
          try {
            ...
            } else if (firebaseResult.skipped) {
              // Firebase not configured - skip silently for demo purposes
              return { success: true, item: item.name, skipped: true };
            } else {
              return { success: false, item: item.name, error: firebaseResult.error };
            }
          } catch (error) {
            console.error(`Error saving ${item.name}:`, error);
            // Don't fail the entire save if one item fails
        ...
        Promise.allSettled(savePromises).then((results) => {
          ...
          if (skipped > 0) {
            console.log(`ℹ️ ${skipped} nutrition log entries skipped (Firebase not configured)`);
          }
          if (failed > 0) {
            console.warn(`⚠️ Failed to save ${failed} nutrition log entries`);
          }
          if (successful > 0) {
            console.log(`✅ Successfully saved ${successful} nutrition log entries`);
          }
        });
      ...
      // Show success message (even if Firebase is not configured, the scan is "saved" locally)
      const firebaseNote = !db ? '\n\nNote: Firebase is not configured. Set up Firebase to enable cloud storage.' : '';
      Alert.alert(
        'Scan Saved',
        'Your scan has been saved.' + firebaseNote,
        [{ text: 'OK' }]
      );
                      ...
                      >
                        ...
                        {food.preparationTime && (
                          <View style={styles.preparationInfo}>
                            <View style={styles.preparationInfoItem}>
                              <Ionicons name="time-outline" size={16} color={Colors.textSecondary} />
                              <Text style={styles.preparationInfoText}>{food.preparationTime}</Text>
                            </View>
                            {food.servings && (
                              <View style={styles.preparationInfoItem}>
                                <Ionicons name="people-outline" size={16} color={Colors.textSecondary} />
                                <Text style={styles.preparationInfoText}>{food.servings} servings</Text>
                              </View>
                            )}
                          </View>
                        )}
                            ...
                            {food.ingredients.map((ingredient, idx) => (
                              <View key={idx} style={styles.ingredientItem}>
                                <View style={styles.ingredientInfo}>
                                  <Ionicons 
                                    name={ingredient.fromFridge ? "checkmark-circle" : "add-circle-outline"} 
                                    size={16} 
                                    color={ingredient.fromFridge ? "#00ff88" : Colors.textSecondary} 
                                  />
                                  <Text style={[
                                    styles.ingredientName,
                                    ingredient.fromFridge && styles.ingredientFromFridge
                                  ]}>
                                    {ingredient.name}
                                  </Text>
                                </View>
                        ...
                        {food.preparationSteps && food.preparationSteps.length > 0 && (
                          <View style={styles.preparationStepsSection}>
                            <View style={styles.sectionHeader}>
